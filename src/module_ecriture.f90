...
CHARACTER(LEN=30), PRIVATE :: FMT,FMT1
...
!---------------------------------------------------------------------------------------------------
    ...
    WRITE(FMT1, "('(A,', I0, '(1pE', I0, '.', I0, ',2X))')") NX / PAS_ECRITURE_SPATIAL+1, PRECISION + 7, PRECISION
...
!---------------------------------------------------------------------------------------------------
    ...
    IF (LIST(17)==1) WRITE(UNT, FMT) "Q_MOINS_AD", Q_MOINS(::PAS_ECRITURE_SPATIAL)
    ...
    IF (LIST(1)==1)  WRITE(UNT, FMT1) "OMEGA_AD  ", OMEGA_AD(::PAS_ECRITURE_SPATIAL),OMEGA_AD(NX)
    IF (LIST(2)==1)  WRITE(UNT, FMT1) "P_AD      ", P_AD(::PAS_ECRITURE_SPATIAL),P_AD(NX)
    IF (LIST(3)==1)  WRITE(UNT, FMT1) "BETA      ", BETA(::PAS_ECRITURE_SPATIAL),BETA(NX)
    IF (LIST(4)==1)  WRITE(UNT, FMT1) "C_S_AD    ", C_S_AD(::PAS_ECRITURE_SPATIAL),C_S_AD(NX)
    IF (LIST(5)==1)  WRITE(UNT, FMT1) "H_AD      ", H_AD(::PAS_ECRITURE_SPATIAL),H_AD(NX)
    IF (LIST(6)==1)  WRITE(UNT, FMT1) "RHO_AD    ", RHO_AD(::PAS_ECRITURE_SPATIAL),RHO_AD(NX)
    IF (LIST(7)==1)  WRITE(UNT, FMT1) "NU_AD     ", NU_AD(::PAS_ECRITURE_SPATIAL),NU_AD(NX)
    IF (LIST(8)==1)  WRITE(UNT, FMT1) "S_AD      ", S_AD(::PAS_ECRITURE_SPATIAL),S_AD(NX)
    IF (LIST(9)==1)  WRITE(UNT, FMT1) "SPEED_AD  ", SPEED_AD(::PAS_ECRITURE_SPATIAL),SPEED_AD(NX)
    IF (LIST(10)==1) WRITE(UNT, FMT1) "TEMP_AD   ", TEMP_AD(::PAS_ECRITURE_SPATIAL),TEMP_AD(NX)
    IF (LIST(11)==1) WRITE(UNT, FMT1) "M_DOT_AD  ", M_DOT_AD(::PAS_ECRITURE_SPATIAL),M_DOT_AD(NX)
    IF (LIST(12)==1) WRITE(UNT, FMT1) "F_Z       ", F_Z(::PAS_ECRITURE_SPATIAL),F_Z(NX)
    IF (LIST(13)==1) WRITE(UNT, FMT1) "P_GAZ_AD  ", P_GAZ_AD(::PAS_ECRITURE_SPATIAL),P_GAZ_AD(NX)
    IF (LIST(14)==1) WRITE(UNT, FMT1) "P_RAD_AD  ", P_RAD_AD(::PAS_ECRITURE_SPATIAL),P_RAD_AD(NX)
    IF (LIST(15)==1) WRITE(UNT, FMT1) "Q_PLUS_AD ", Q_PLUS_AD(::PAS_ECRITURE_SPATIAL),Q_PLUS_AD(NX)
    IF (LIST(16)==1) WRITE(UNT, FMT1) "Q_ADV_AD  ", Q_ADV_AD(::PAS_ECRITURE_SPATIAL),Q_ADV_AD(NX)
    IF (LIST(17)==1) WRITE(UNT, FMT1) "Q_MOINS_AD", Q_MOINS_AD(::PAS_ECRITURE_SPATIAL),Q_MOINS_AD(NX)
    IF (LIST(18)==1) WRITE(UNT, FMT1) "TAU_EFF   ", TAU_EFF(::PAS_ECRITURE_SPATIAL),TAU_EFF(NX)
    IF (LIST(19)==1) WRITE(UNT, FMT1) "K_FF      ", KAPPA_FF(::PAS_ECRITURE_SPATIAL),KAPPA_FF(NX)
    IF (LIST(20)==1) WRITE(UNT, FMT1) "E_FF      ", EPSILON_FF(::PAS_ECRITURE_SPATIAL),EPSILON_FF(NX)
...
!---------------------------------------------------------------------------------------------------
    ...
    ! TEMPS
    COUNT = COUNT + 1
    IF (MODULO(COUNT, PAS_ECRITURE_TEMPOREL) /= 1) RETURN
    ...
    IF (LIST(1)==1)  WRITE(UNT, FMT1) "OMEGA     ", OMEGA(::PAS_ECRITURE_SPATIAL),OMEGA(NX)
    IF (LIST(2)==1)  WRITE(UNT, FMT1) "P         ", P(::PAS_ECRITURE_SPATIAL),P(NX)
    IF (LIST(3)==1)  WRITE(UNT, FMT1) "BETA      ", BETA(::PAS_ECRITURE_SPATIAL),BETA(NX)
    IF (LIST(4)==1)  WRITE(UNT, FMT1) "C_S       ", C_S(::PAS_ECRITURE_SPATIAL),C_S(NX)
    IF (LIST(5)==1)  WRITE(UNT, FMT1) "H         ", H(::PAS_ECRITURE_SPATIAL),H(NX)
    IF (LIST(6)==1)  WRITE(UNT, FMT1) "RHO       ", RHO(::PAS_ECRITURE_SPATIAL),RHO(NX)
    IF (LIST(7)==1)  WRITE(UNT, FMT1) "NU        ", NU(::PAS_ECRITURE_SPATIAL),NU(NX)
    IF (LIST(8)==1)  WRITE(UNT, FMT1) "SIGMA     ", SIGMA(::PAS_ECRITURE_SPATIAL),SIGMA(NX)
    IF (LIST(9)==1)  WRITE(UNT, FMT1) "SPEED     ", SPEED(::PAS_ECRITURE_SPATIAL),SPEED(NX)
    IF (LIST(10)==1) WRITE(UNT, FMT1) "TEMP      ", TEMP(::PAS_ECRITURE_SPATIAL),TEMP(NX)
    IF (LIST(11)==1) WRITE(UNT, FMT1) "M_DOT     ", M_DOT(::PAS_ECRITURE_SPATIAL),M_DOT(NX)
    IF (LIST(12)==1) WRITE(UNT, FMT1) "F_Z       ", F_Z(::PAS_ECRITURE_SPATIAL),F_Z(NX)
    IF (LIST(13)==1) WRITE(UNT, FMT1) "P_GAZ     ", P_GAZ(::PAS_ECRITURE_SPATIAL),P_GAZ(NX)
    IF (LIST(14)==1) WRITE(UNT, FMT1) "P_RAD     ", P_RAD(::PAS_ECRITURE_SPATIAL),P_RAD(NX)
    IF (LIST(15)==1) WRITE(UNT, FMT1) "Q_PLUS    ", Q_PLUS(::PAS_ECRITURE_SPATIAL),Q_PLUS(NX)
    IF (LIST(16)==1) WRITE(UNT, FMT1) "Q_ADV     ", Q_ADV(::PAS_ECRITURE_SPATIAL),Q_ADV(NX)
    IF (LIST(17)==1) WRITE(UNT, FMT1) "Q_MOINS   ", Q_MOINS(::PAS_ECRITURE_SPATIAL),Q_MOINS(NX)
    IF (LIST(18)==1) WRITE(UNT, FMT1) "TAU_EFF   ", TAU_EFF(::PAS_ECRITURE_SPATIAL),TAU_EFF(NX)
    IF (LIST(19)==1) WRITE(UNT, FMT1) "K_FF      ", KAPPA_FF(::PAS_ECRITURE_SPATIAL),KAPPA_FF(NX)
    IF (LIST(20)==1) WRITE(UNT, FMT1) "E_FF      ", EPSILON_FF(::PAS_ECRITURE_SPATIAL),EPSILON_FF(NX)
