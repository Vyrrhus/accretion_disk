...
REAL(KIND=xp), PRIVATE, PARAMETER    :: PREC_DICHO  = 1e-12_xp
INTEGER, PRIVATE, PARAMETER          :: LIMIT_DICHO = 1000
...
!---------------------------------------------------------------------------------------------------
    ...
    CALL calc_H2(TEMP_S_AD, X_AD(ipos), OMEGA_AD(ipos), Sa_AD, Ha_AD)
    CALL calc_H2(TEMP_S_AD, X_AD(ipos), OMEGA_AD(ipos), Sb_AD, Hb_AD)
    ...
    IF (mince .eqv. .true.) THEN
        ...
        DO WHILE(eps > PREC_DICHO .and. counter < LIMIT_DICHO)
            ...
            CALL calc_H2(TEMP_S_AD, X_AD(ipos), OMEGA_AD(ipos), Sc_AD, Hc_AD)
...
!---------------------------------------------------------------------------------------------------
   REAL(KIND=xp), INTENT(in)    :: S_S_AD                                               !! Température
   ...
   ! Calcul de H aux points A et B
   CALL calc_H2( Temp_a_AD, x_ad(ipos), Omega_S_AD, S_S_AD, Ha_AD )
   CALL calc_H2( Temp_b_AD, x_ad(ipos), Omega_S_AD, S_S_AD, Hb_AD )
   ...
   Fb = Q_plus_b - Q_moins_b
    ...
    DO WHILE(eps > PREC_DICHO .and. counter < LIMIT_DICHO)
        ...
        CALL calc_H2( Temp_c_AD, x_ad(ipos), Omega_S_AD, S_S_AD, Hc_AD )
